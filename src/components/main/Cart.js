    ...
    return (
      <View>
        <View style={{marginTop:15, marginLeft:8}}>
          <Text style={{fontSize:20,fontWeight: 'bold', paddingLeft:5, paddingBottom:9}}>Carrinho</Text>
        </View>
        <View style={{marginTop:15, marginLeft:8}}>
          <Text style={{fontSize:17,fontWeight: 'bold', paddingLeft:5, paddingBottom:9}}>Itens</Text>
        </View>
        {this.state.dataSource!==null ? this.state.dataSource.map((item, i) => {
          return(
          <View key={item.id}>
          {item.itens.map((itens, j) => {
            return(
            <Card>
<View key={item.id}>
                            ...
                            <View style={{flexWrap: 'wrap', 
                              ...
                              <Text style={{color:'#5E5B5B'}}> {itens.observacoes}</Text>
                            </View>
                            <View style={{flexWrap: 'wrap', 
                                          alignItems: 'flex-start',
                                          flexDirection:'row'}}>
                              <Text style={{fontWeight:'bold', color:'#000',marginLeft:25}}>Quantidade:</Text>
                              <Text style={{color:'#5E5B5B'}}> {itens.quantidade}</Text>
                            </View>
                            <View style={{flexWrap: 'wrap', 
                                          alignItems: 'flex-start',
                                          flexDirection:'row'}}>
                              <Text style={{fontWeight:'bold', color:'#000',marginLeft:25}}>Preço:</Text>
                              <Text style={{color:'#5E5B5B'}}>R$ {itens.valor}</Text>
                            </View>
                          </View>
            </Card>
          )})}
          <View style={{alignItems:'flex-end', marginRight:'10%', marginTop:10}}>
            <Text style={{fontWeight:'bold', color:'#f00', alignSelf:'flex-end', marginRight:10}}>Total: R$ {item.valor_total}</Text>
          </View>
          </View>
        )}): <View />}
            <Button
